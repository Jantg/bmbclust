...
n_partitions_Nk <- function(k,N,gamma_k){
  ...
  c_K_k <- function(k,N,gamma_k){
    if(k == 1)  return(matrix(init_vec))
    if(length(res) < k) res <<- "length<-" (res,k)
    if(!is.null(res[[k]])) return(res[[k]])
...
setClass("bmbclust",
         ...
         prototype = list(
           ...
           prior.alpha.parameters = list(df1 =6.0, df2 = 5.0),
           max.k = 150,
           fix.k = FALSE
         ))
...
setClass("BMBCMx_gauss",
         slots = c(
           data = "matrixOrdata.frame",
           component.priors_hyper = "list",
           init.meanmat = "array",
           init.covmat = "array",
           current.meanmat = "array",
           current.covmat = "array",
           hyperprior.params = "list",
           component.params = "list"
         ),
         prototype = list(
           hyperprior.params = list(nu1 = 0.4, nu2=0.4)
         ),
         validity = function(object){
...
bmbclust <- function(data,data.type,component.density = NULL,prior.k = NULL,prior.alpha = NULL,prior.k.parameters = NULL,prior.alpha.parameters = NULL,
                     init.k = NULL, max.k = NULL, sig_alpha = NULL,fix.k = NULL){

  if(!is.matrix(data)&&!is.data.frame(data)) stop("data should be of type matrix or data.frame")
  is_mixed <- length(unique(simplify2array(lapply(as.data.frame(data),class)))) != 1
    if(is_mixed){
      stop("mixed type data is currently not supported, if your data is not a mixed type, please check column types")
    }
  switch(data.type,
         real.valued = {
           if(is.null(component.density)) component.density <- "gaussian"
         },
         count = {
           if(is.null(component.density)) component.density <- "poisson"
         },
         categorical = {
           if(is.null(component.density)) component.density <- "categorical"
         },
         stop("supplied data.type argument does not match with any of the three types (categorical, count and real-valued) currently supported")
  )
  args_list <- as.list(match.call())
  args_list <- args_list[2:length(args_list)] # the first element is function name "bmbclust" so remove this
  bmbclust_slots <- append(args_list[names(args_list)%in% setdiff(slotNames("bmbclust"),c("component.density","data.type"))],list(component.density = component.density, data.type = data.type))
  mixture.obj_slots <- args_list[!(names(args_list)%in% slotNames("bmbclust"))]
   
  switch(component.density,
         gaussian = {
           mixture.obj <- do.call("new",append("BMBCMx_gauss",mixture.obj_slots))
           return_obj <- do.call("new",append("bmbclust",append(bmbclust_slots,list(mixture.obj = mixture.obj))))
         },
         poisson = {
           mixture.obj <- do.call("new",append("BMBCMx_pois",mixture.obj_slots))
           return_obj <- do.call("new",append("bmbclust",append(bmbclust_slots,list(mixture.obj = mixture.obj))))
         },
         categorical = {
           mixture.obj <- do.call("new",append("BMBCMx_categ",mixture.obj_slots))
           mixture.obj@col_levels <- lapply(setNames(colnames(data),colnames(data)),function(x){
             levels(do.call("$",args = list(data,x)))
           })
           return_obj <- do.call("new",append("bmbclust",append(bmbclust_slots,list(mixture.obj = mixture.obj))))
         },
         stop("supplied component.density argument does not match with any of the three densities (gaussian,poisson and categorical) currently supported")
  )
  return(return_obj)
}
...
setMethod("simulate",
          ...
          function(bmbclust_obj,sampler = "Telescope",n.iter = 1e5, n.burnin = 1e5,n.thin = 1,n.chains = 1,verbose = TRUE,
            ...
            Telescope_sampler(bmbclust_obj,n.iter = n.iter, n.burnin = n.burnin, n.thin = n.thin,
                              n.chains = n.chains,verbose = verbose,show.prior.Kplus = show.prior.Kplus)
            }else{
...
setMethod("Telescope_sampler",
          signature(bmbclust_obj = "bmbclust"),
          function(bmbclust_obj,n.iter,n.burnin,n.thin,n.chains, verbose,show.prior.Kplus){
            # prepare data and initial specifications for K and alpha
            if(length(bmbclust_obj@posteriors)==0){
              idx <- 1
            }else{
              idx <- length(bmbclust_obj@posteriors$Ks)+1
            }
            ...
            bmbclust_obj <- Init_sampler(bmbclust_obj@mixture.obj,bmbclust_obj)
            bmbclust_obj <- Init_params(bmbclust_obj@mixture.obj,bmbclust_obj)
            
            if(length(bmbclust_obj@posteriors)==0){
              alpha <- bmbclust_obj@mixture.obj@init.alpha
              K <- bmbclust_obj@mixture.obj@init.k
              inits <- grep("init.*",names(attributes(bmbclust_obj@mixture.obj)),value = TRUE)
              init_list <- lapply(setNames(inits,inits),function(x) do.call("@",list(bmbclust_obj@mixture.obj,x)))
            ...
            bmbclust_obj <- Init_post_list(bmbclust_obj@mixture.obj,bmbclust_obj,n.iter,n.thin)
            
            for(n in 1:(n.iter + n.burnin)){
              tmp <- Index_update(bmbclust_obj@mixture.obj,bmbclust_obj,etas,K)
              S <- tmp[[1]]
              alloc_probs <- tmp[[2]]
              ...
              bmbclust_obj <- Reorder_components(bmbclust_obj@mixture.obj,bmbclust_obj,mapping,K)
              S <- sapply(S, function(x) which(x == as.numeric(names(mapping))))
              N <- c(tabulate(S), rep(0, K - K_plus))
              
              if((n>n.burnin) && ((n- n.burnin)%% n.thin == 0)){
                loglik <- sum(alloc_probs)#eval_partition_loglik(bmbclust_obj,K,S,etas)
                bmbclust_obj <- record_post_draws(bmbclust_obj@mixture.obj,bmbclust_obj,idx,etas,K,K_plus,alpha,N,loglik,S,alloc_probs)
              }
              
              bmbclust_obj <- Component_update(bmbclust_obj@mixture.obj,bmbclust_obj,K_plus,N,S)
              if(!bmbclust_obj@fix.k){
              unnorm_logprob <- sapply(K_plus:K_max,function(k){
                do.call(bmbclust_obj@prior.k,append(list(x=k-1),append(bmbclust_obj@prior.k.parameters,list(log = TRUE))))+
              ...
              tmp <- sapply(c(alpha_prop,alpha),function(a){
                do.call(bmbclust_obj@prior.alpha,append(list(x=a),append(bmbclust_obj@prior.alpha.parameters,list(log=TRUE))))+
              ...
              bmbclust_obj <- Sample_component_params(bmbclust_obj@mixture.obj,bmbclust_obj,K_plus,K)
              ...
              if((n>n.burnin) && ((n- n.burnin)%% n.thin == 0)){
                ...
                if(verbose){
                  ...
                  # col = 100 is blue
                  matplot(log(bmbclust_obj@posteriors$alphas[1:idx]/bmbclust_obj@posteriors$Ks[1:idx]), type = "l", lty = 1,col = 100, ylab = "log(alpha/K)",
                  ...
                  matplot(bmbclust_obj@posteriors$Kps[1:idx]/bmbclust_obj@posteriors$Ks[1:idx],type = "l",lty = 1,col = 100,lwd = 0.3, ylab = "Kplus/K",
                          main = "Traceplot of Kplus/K")
                  par(mar = c(3,0,3,0))
                  hist_Kp_K <- hist(bmbclust_obj@posteriors$Kps[1:idx]/bmbclust_obj@posteriors$Ks[1:idx],plot = FALSE)
                  ...
                  rownames(temporal_partitions) <- if(nrow(temporal_partitions) ==1){c("current")}else{c(paste("m-",(nrow(temporal_partitions)-1):1),"current")}
                  ...
                  matplot(bmbclust_obj@posteriors$Kps[1:idx],type = "l",lty = 1,col = 100, ylab = "Kplus",main = "Traceplot of Kplus")
                  par(mar = c(3,0,3,0))
                  ...
                  if(show.prior.Kplus){
                    ...
                    lines(exp(probs_Kplus)[yaxis],yaxis-min(bmbclust_obj@posteriors$Kps[1:idx])+0.5,col="red",type = "o")
                  }
                  #curve(exp(probs_Kplus[x]),from = max(bmbclust_obj@posteriors$Kps[1:idx]),
                  #      to = max(bmbclust_obj@posteriors$Kps[1:idx]),type = "o", col = "red",
                  #      n = (max(bmbclust_obj@posteriors$Kps[1:idx])-min(bmbclust_obj@posteriors$Kps[1:idx])+1),
                  #      add = TRUE,lwd = 2)
                  par(mar = c(3,3,3,0))
                  thresh <- quantile(bmbclust_obj@posteriors$loglik[1:idx],probs = c(0.025))
                  ...
                  # histogram of K_plus
                  #hist(bmbclust_obj@posteriors$Kps[1:idx],freq = FALSE,main = "Histogram of K_plus")
...
setMethod("compute_prior_Kplus",
          ...
          function(bmbclust_obj,idx,N){
            lprobs_Kplus <- sapply(1:max(bmbclust_obj@posteriors$Kps[1:idx]),function(kp){ 
              matrixStats::logSumExp(sapply(kp:bmbclust_obj@max.k,function(KK){
                gamma_k <- bmbclust_obj@posteriors$alphas[idx]/KK
                val <- lfactorial(KK)-lfactorial(KK-kp)+lgamma(gamma_k*KK)-lgamma(N+gamma_k*KK)-kp*lgamma(gamma_k)+
                  n_partitions_Nk(kp,N,gamma_k)[1]+
                  do.call(bmbclust_obj@prior.k,append(list(x=KK-1),append(bmbclust_obj@prior.k.parameters,list(log = TRUE))))
                val
...
setGeneric("Init_sampler",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("Init_sampler"))

setMethod("Init_sampler",
          signature(mixture_obj = "BMBCMx_gauss"),
          function(mixture_obj,bmbclust_obj){
            if(length(mixture_obj@component.priors_hyper)==0){
            ...
            b0 <- m0
            mixture_obj@component.priors_hyper <- list(c0=c0,g0=g0,G0=G0,C0_j=C0_j,B0=B0,inv_B0=inv_B0,b0=b0,R=R)
            }
...
setMethod("Init_sampler",
          ...
          function(mixture_obj,bmbclust_obj){
            ...
            G0 <- g0/b0
            mixture_obj@component.priors_hyper <- list(a0 = a0, b0 = b0, g0 = g0, G0 = G0)
            }
            bmbclust_obj@mixture.obj <- mixture_obj
            return(bmbclust_obj)
          }
...
setMethod("Init_sampler",
        ...
        function(mixture_obj,bmbclust_obj){
          if((length(mixture_obj@component.priors_hyper)==0)&& (all(dim(mixture_obj@data_I) ==0))){
          data <- mixture_obj@data
          cat<- apply(data,2,function(x) max(as.numeric(x))-min(as.numeric(x))+1)
          a0 <- 1
          data_I <- Reduce(cbind,
                 lapply(1:ncol(data),function(col_i){
                   simplify2array(lapply(1:cat[col_i],function(cat_i){
                     as.numeric(data[,col_i]) == cat_i
                     }))
                   }))
          mixture_obj@component.priors_hyper <- list(cat = cat, a0=a0)
...
setGeneric("Init_params",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("Init_params"))

setMethod("Init_params",
          signature(mixture_obj = "BMBCMx_gauss"),
          function(mixture_obj,bmbclust_obj){
            if((length(mixture_obj@current.meanmat)==0)&&(length(mixture_obj@current.covmat)==0)){
            data <- mixture_obj@data
            r <- ncol(data)
            ...
            if(length(mixture_obj@init.meanmat)==0){
              ...
              init.meanmat <- array(clst$centers,dim = c(K_0,r,1))
              mixture_obj@init.meanmat <- init.meanmat
            }
            if(length(mixture_obj@init.covmat)==0){
              init.covmat <- array(runif(r*r*K_0),dim = c(K_0,r,r))
              init.covmat <- array(t(apply(init.covmat,1,function(x) crossprod(x))),dim = c(K_0,r,r))
...
setMethod("Init_params",
          ...
          function(mixture_obj,bmbclust_obj){
            ...
            if(length(mixture_obj@init.meanmat)==0){
              ...
              S_0 <- clst$cluster
              init.meanmat <- Reduce(rbind,
                lapply(1:length(cat),function(col_i){
                  # matrix of number of categories in col_i times number of clusters
                  # filled with empirical probabilities
                  simplify2array(lapply(1:K_0,function(k){
                    tabulate(data[S_0 == k,col_i],cat[col_i])/sum(S_0 == k)
                  }))
                })
              )
              mixture_obj@init.meanmat <- init.meanmat
...
setGeneric("Init_post_list",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("Init_post_list"))

setMethod("Init_post_list",
          signature(mixture_obj = "BMBCMx_gauss"),
          function(mixture_obj,bmbclust_obj,n.iter,n.thin){
            if(length(bmbclust_obj@posteriors) == 0){
              bmbclust_obj@posteriors <- list(eta = list(), Ks = numeric(n.iter/n.thin), Kps = numeric(n.iter/n.thin),
                                             alphas = numeric(n.iter/n.thin),comp_means = list(),alloc_vectors = list(),
              ...
              bmbclust_obj@posteriors <- list(eta = bmbclust_obj@posteriors$eta,alphas =c(bmbclust_obj@posteriors$alphas,numeric(n.iter/n.thin)),Ks = c(bmbclust_obj@posteriors$Ks,numeric(n.iter/n.thin)),
                                              Kps = c(bmbclust_obj@posteriors$Kps,numeric(n.iter/n.thin)),comp_means = bmbclust_obj@posteriors$comp_means,
                                              alloc_vectors = bmbclust_obj@posteriors$alloc_vectors, comp_covs = bmbclust_obj@posteriors$comp_covs,
                                              bs = bmbclust_obj@posteriors$bs, Bs = bmbclust_obj@posteriors$Bs,max_kplus = bmbclust_obj@posteriors$max_kplus,
                                              partition = bmbclust_obj@posteriors$partition,MAP = bmbclust_obj@posteriors$MAP, columns = bmbclust_obj@posteriors$columns,
                                              loglik = c(bmbclust_obj@posteriors$loglik,numeric(n.iter/n.thin)),alloc_probs = bmbclust_obj@posteriors$alloc_probs)
            }
            return(bmbclust_obj)
          })
...
setMethod("Init_post_list",
          ...
          function(mixture_obj,bmbclust_obj,n.iter,n.thin){
            if(length(bmbclust_obj@posteriors) == 0){
              bmbclust_obj@posteriors <- list(eta = list(), Ks = numeric(n.iter/n.thin), Kps = numeric(n.iter/n.thin),
                                              alphas = numeric(n.iter/n.thin),comp_means = list(),alloc_vectors = list(),
                                              max_kplus = 0,partition = list(), MAP = list(),columns = NA_character_,
                                              loglik = numeric(n.iter/n.thin),alloc_probs = list())
            }else{
              bmbclust_obj@posteriors <- list(eta = bmbclust_obj@posteriors$eta, alphas =c(bmbclust_obj@posteriors$alphas,numeric(n.iter/n.thin)),Ks = c(bmbclust_obj@posteriors$Ks,numeric(n.iter/n.thin)),
                                              Kps = c(bmbclust_obj@posteriors$Kps,numeric(n.iter/n.thin)),comp_means = bmbclust_obj@posteriors$comp_means,
                                              alloc_vectors = bmbclust_obj@posteriors$alloc_vectors,max_kplus = bmbclust_obj@posteriors$max_kplus,
                                              partition = bmbclust_obj@posteriors$partition,MAP = bmbclust_obj@posteriors$MAP, columns = bmbclust_obj@posteriors$columns,
                                              loglik = c(bmbclust_obj@posteriors$loglik,numeric(n.iter/n.thin)),alloc_probs = bmbclust_obj@posteriors$alloc_probs)
...
setGeneric("Index_update",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("Index_update"))
...
setMethod("Index_update",
          ...
          function(mixture_obj,bmbclust_obj,etas,K){
            ...
            eta_prod_dens <- simplify2array(
              lapply(1:K,function(k){
                apply(data_I,1,function(x) sum(log(meanmat[x,k])))+etas[k]
              })
            )
            gumbel_noise <- matrix(evd::rgumbel(K*nrow(data_I)),nrow = nrow(data_I))
            S <- apply(eta_prod_dens+gumbel_noise,1,function(x) which.max(x))
            return(list(S,eta_prod_dens))
          })

setGeneric("Reorder_components",
           function(mixture_obj,bmbclust_obj,mapping,...) standardGeneric("Reorder_components"))
...
setMethod("Reorder_components",
          ...
          function(mixture_obj,bmbclust_obj,mapping,K){
            ...
            mixture_obj@current.meanmat <- array(mixture_obj@current.meanmat[as.numeric(names(mapping)), , ], dim = c(K, r, 1))
            mixture_obj@current.covmat <- array(mixture_obj@current.covmat[as.numeric(names(mapping)), , ], dim = c(K, r, r))
            bmbclust_obj@mixture.obj <- mixture_obj
            return(bmbclust_obj)
          }
          )
...
setMethod("Reorder_components",
          ...
          function(mixture_obj,bmbclust_obj,mapping,K){
            ...
            mixture_obj@current.meanmat <- mixture_obj@current.meanmat[as.numeric(names(mapping)), ,drop=FALSE]
            bmbclust_obj@mixture.obj <- mixture_obj
            return(bmbclust_obj)
          }
)
...
setGeneric("Component_update",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("Component_update"))
...
setMethod("Component_update",
          ...
          function(mixture_obj,bmbclust_obj,K_plus,N,S){
            ...
            b0 <- crossprod(chol(B0),rnorm(r)) + Reduce("+", mu)/K_plus
            mixture_obj@component.priors_hyper <- list(c0=c0,g0=g0,G0=G0,C0_j=C0_j,B0=B0,inv_B0=inv_B0,b0=b0,R=R)
            mixture_obj@component.params <- list(mu = mu, sig = sig, bk = bk, Bk = Bk)
            bmbclust_obj@mixture.obj <- mixture_obj
            return(bmbclust_obj)
          }
          )
...
setMethod("Component_update",
          ...
          function(mixture_obj,bmbclust_obj,K_plus,N,S){
            ...
            mixture_obj@component.params <- list(mu = mus, ak = ak, bk = bk)
            bmbclust_obj@mixture.obj <- mixture_obj
            return(bmbclust_obj)
          })
...
setGeneric("Sample_component_params",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("Sample_component_params"))
...
setGeneric("eval_partition_loglik",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("eval_partition_loglik"))
...
setMethod("eval_partition_loglik",
          ...
          function(mixture_obj,bmbclust_obj,K,S,etas){
            ...
            loglik <- simplify2array(lapply(1:nrow(data),function(idx){
              dMvn(matrix(data[idx,],ncol = r),matrix(meanmat[S[idx],,],ncol = r),matrix(covmat[S[idx],,],ncol = r),log = TRUE)+etas[S[idx]]
            }))
            return(sum(loglik))
          })
...
setMethod("eval_partition_loglik",
          ...
          function(mixture_obj,bmbclust_obj,K,S,etas){
            ...
            loglik <- simplify2array(lapply(1:nrow(data),function(idx){
              dpois(matrix(data[idx,],ncol = r),matrix(meanmat[S[idx],],ncol = r),log = TRUE)+etas[S[idx]]
            }))
            return(sum(loglik))
          })
...
setMethod("eval_partition_loglik",
          ...
          function(mixture_obj,bmbclust_obj,K,S,etas){
            ...
            #loglik <- simplify2array(lapply(1:nrow(data_I), function(idx){ 
...
setGeneric("record_post_draws",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("record_post_draws"))
...
setMethod("record_post_draws",
          ...
          function(mixture_obj,bmbclust_obj,idx,etas,K,K_plus,alpha,N,loglik,S,alloc_probs){
            ...
            bmbclust_obj@posteriors$comp_means[[idx]] <-lapply(1:ncol(cumcat_and_dif),function(idx){
...
setMethod("tolabel.switching",
          ...
          function(bmbclust_obj,K_MAP = NULL,methods = NULL){
            ...
            return_list <- return_label_switching_args(bmbclust_obj@mixture.obj,bmbclust_obj,MAP_idx,K_MAP)
...
setGeneric("return_label_switching_args",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("return_label_switching_args"))

setMethod("return_label_switching_args",
          signature(mixture_obj = "BMBCMx_gauss"),
          function(mixture_obj,bmbclust_obj,MAP_idx,K_MAP){
            mcmc.mean <- simplify2array(lapply(bmbclust_obj@posteriors$comp_means[MAP_idx],function(x){
              if(is.null(dim(x[,,1]))){matrix(x[,,1][1:K_MAP])}else{x[,,1][1:K_MAP,]}
            }))
            mcmc.mean <- aperm(mcmc.mean,c(3,1,2))
            mcmc.cov <- simplify2array(lapply(bmbclust_obj@posteriors$comp_covs[MAP_idx],function(x) t(simplify2array(lapply(1:K_MAP,function(k)as.vector(x[k,,]))))))
...
setMethod("identify",
          ...
          function(bmbclust_obj,K_MAP=NULL,perms=NULL,nonperm=NULL){
            if(is.null(perms)){
              ...
              tmp <- identify_w_kmeans(bmbclust_obj@mixture.obj,bmbclust_obj,K_MAP)
            ...
            bmbclust_obj <- reorder_and_record(bmbclust_obj@mixture.obj,bmbclust_obj,K_MAP,perm_idx,clst_perm_idx)
...
setGeneric("reorder_and_record",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("reorder_and_record"))
...
setMethod("reorder_and_record",
          ...
          function(mixture_obj,bmbclust_obj,K_MAP,perm_idx,clst_perm_idx){
            res_ordered_mean <- lapply(1:nrow(perm_idx),function(x){
              if(ncol(bmbclust_obj@mixture.obj@data)==1){
                bmbclust_obj@posteriors$comp_means[[perm_idx[x,2]]][,,1][clst_perm_idx[x,]] 
              }else{
                bmbclust_obj@posteriors$comp_means[[perm_idx[x,2]]][,,1][clst_perm_idx[x,],] 
              }
            })
            ...
            res_ordered_cov <- lapply(1:nrow(perm_idx),function(x){
              if(ncol(bmbclust_obj@mixture.obj@data)==1){
                bmbclust_obj@posteriors$comp_covs[[perm_idx[x,2]]][clst_perm_idx[x,],,,drop=FALSE]
              }else{
                bmbclust_obj@posteriors$comp_covs[[perm_idx[x,2]]][clst_perm_idx[x,],,]
              }})
...
setGeneric("identify_w_kmeans",
           function(mixture_obj,bmbclust_obj,...) standardGeneric("identify_w_kmeans"))
...
setMethod("identify_w_kmeans",
          ...
          function(mixture_obj,bmbclust_obj,K_MAP){
            ...
            res_df_covdet <- Reduce(rbind,
                                    lapply(bmbclust_obj@posteriors$comp_covs[MAP_idx],function(x){
                                      if(is.null(dim(x[,,1]))){
...
setGeneric("compute_CI",
           function(mixture_obj,...) standardGeneric("compute_CI"))
...
setMethod("summary",
          ...
          function(object,quantiles=NULL){
            ...
            CIs <- compute_CI(object@mixture.obj,meanmat,K_MAP,quantiles)
            invisible(
...
setMethod("plot",
          ...
          function(x,K_MAP = NULL){
            ...
            if(length(bmbclust_obj@post.identification) == 0){
              MAP_idx <- bmbclust_obj@posteriors$Kps == K_MAP
              df_pointprocess <- prepare_df(bmbclust_obj@mixture.obj,bmbclust_obj,K_MAP,MAP_idx)
...
setGeneric("prepare_df",
           function(mixture_obj,...) standardGeneric("prepare_df"))
